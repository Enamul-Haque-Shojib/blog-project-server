  ...
  try {
    ...
    if (!user) {
      throw new AppError(
        ...
        `You are not authorized by your ${email}`,
      );
    }
    ...
    if (!populatedBlog) {
      throw new AppError(
        404,
        'populate_error of author',
        'Failed to retrieve created blog',
      );
    }
...
const updateSingleBlogIntoDB = async (id: string, payload: Partial<TBlog>) => {
  ...
  if (await BlogModel.isBlogDeleted(id)) {
    throw new AppError(
      ...
      `Blog does not exists this ${id} because it is already deleted`,
    );
  }
